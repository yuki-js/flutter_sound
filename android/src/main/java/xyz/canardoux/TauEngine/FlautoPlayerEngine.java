...
class FlautoPlayerEngine extends FlautoPlayerEngineInterface {
	...
	// Synchronization lock for audioTrack access to prevent race conditions
	private final Object audioLock = new Object();
	// Flag to indicate if the player is stopping (volatile for thread visibility)
	...
	*/
	...
	class FeedThread extends Thread {
		...
		public void run() {
			// Check if stopping before attempting to write
			if (isStopping) {
				return;
			}
			...
			// Call needSomeFood outside synchronized block to avoid holding lock during callback
			if (!isStopping && mSession != null) {
	...
	void _stop() {
		// Set stopping flag first to prevent new feed operations from starting
		isStopping = true;
